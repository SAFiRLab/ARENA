...
public:
    ...
    TraversabilityCostCapability(std::shared_ptr<grid_map::GridMap> a_global_map)
        : global_map_(a_global_map)
    {
        if (!global_map_)
            throw std::runtime_error("TraversabilityCostCapability: global grid_map is null.");
    };
    ...
    double getTraversabilityCost(const Eigen::Vector2d& a_point) const
    {
        ...
        if (!global_map_->isInside(position))
            throw std::out_of_range("TraversabilityCostCapability::getTraversabilityCost() point is outside of the grid map.");

        // Index from position
        grid_map::Index index;
        global_map_->getIndex(position, index);
        return double(global_map_->at("cost", index));
    };
    ...
    double getOccupancy(const Eigen::Vector2d &a_point) const
    {
        ...
        grid_map::Index index;
        global_map_->getIndex(position, index);
        return double(global_map_->at("occupancy", index));
    }
    ...
    double getElevationAt(const Eigen::Vector2d &a_point) const
    {
        ...
        grid_map::Index index;
        global_map_->getIndex(position, index);
        return double(global_map_->at("elevation", index));
    }
...
public:
    ...
    bool isOccupied(const Eigen::VectorXd& a_point) const override
    {
        ...
        double occupancy_value = global_map_->at("occupancy", index);
        return (occupancy_value >= 0.5f) ? true : false;
    };
    ...
    bool isClearance(const Eigen::VectorXd& a_point, double a_clearance_radius) const override
    {
        ...
        for (int dx = -radius_cells; dx <= radius_cells; ++dx)
        {
            for (int dy = -radius_cells; dy <= radius_cells; ++dy)
            {
                ...
                global_map_->getPosition(neighbor_index, neighbor_pos);
                if (!global_map_->isInside(neighbor_pos)) continue;

                if (!global_map_->isValid(neighbor_index, "occupancy"))
                    continue;

                if (global_map_->at("occupancy", neighbor_index) >= 0.5f)
                    return false;
            }
        }
    ...
    Eigen::MatrixXd getMapBounds() const override
    {
        ...
        bounds(0, 0) = global_map_->getPosition().x() - global_map_->getLength().x() / 2.0; // min_x
        bounds(0, 1) = global_map_->getPosition().y() - global_map_->getLength().y() / 2.0; // min_y
        bounds(1, 0) = global_map_->getPosition().x() + global_map_->getLength().x() / 2.0; // max_x
        bounds(1, 1) = global_map_->getPosition().y() + global_map_->getLength().y() / 2.0; // max_y

        return bounds;
    };

    std::shared_ptr<grid_map::GridMap> getGlobalMap() const { return global_map_; };
    ...
    void setGlobalMap(std::shared_ptr<grid_map::GridMap> a_global_map)
    {
        ...
        if (!cap)
        {
            ...
            registerCapability(std::make_shared<TraversabilityCostCapability>(global_map_));
        }
    }
